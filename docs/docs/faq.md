...
[`runtime.ServeMux`](https://pkg.go.dev/github.com/sunguohua/grpc-gateway/runtime?tab=doc#ServeMux). Since `runtime.ServeMux` is just a standard [`http.Handler`](http://golang.org/pkg/http#Handler), you can easily write a custom wrapper of `runtime.ServeMux`, leveraged with existing third-party libraries in Go (e.g. [gateway main.go program](https://github.com/sunguohua/grpc-gateway/blob/main/examples/internal/gateway/main.go).
